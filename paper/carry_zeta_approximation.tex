...
\item
  ...
  transcendental in \(\{\ln 2, \ln 3, 1\}\) (PSLQ). The effective rate
  \(\rho(K) = |\Delta(K)/\Delta(K{-}1)|\) approaches \(1/2\) for \(K \leq 17\)
  (\(\rho(17) = 0.505\)), then drops below \(1/2\) at \(K = 18\)--\(21\)
  (\(\rho(21) = 0.333\)), indicating a super-geometric phase where \(P(K)\)
  passes through its maximum magnitude {[}E; experiment B30{]}. The base rate
  \(1/2\) is the Diaconis--Fulman eigenvalue \(\lambda_2 = 1/b\), proved for all
  bases \(b\) {[}experiment B29; G, G04{]}.
\end{itemize}
...
. Since \(\lfloor W \rfloor \in \{2, 3\}\) on \([2, 4)\), the only non-zero
contribution to \(c_{\mathrm{top}-1} - 1\) comes from \(\lfloor W \rfloor = 3\),
i.e., \(W \geq 3\). Hence \(P_e \cdot c_1^{(e)} = P(W \geq 3)\). The integration
domain \(\{(x,y) \in [0,1)^2 : (1+x)(1+y) \geq 3\}\) has boundary
...
\item
  ...
  conditional is transcendental in \(\{\ln 2, \ln 3, 1\}\) (PSLQ-proved). The
  convergence \(c_1(K) \to \pi/18\) is governed by the Diaconis--Fulman
  eigenvalue \(\lambda_2 = 1/b\), proved for general base \(b\) {[}experiment
  B29; G, G04{]}. At \(K = 18\)--\(21\), the effective rate drops below \(1/2\)
  (super-geometric phase). The appearance of \(\pi\) is explained by the Angular
  Uniqueness Theorem {[}G, Theorem{]}. A formal proof of \(c_1 = \pi/18\)
